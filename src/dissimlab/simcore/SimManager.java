...
public class SimManager {
	...
	private SimMode simMode = SimMode.ASAP;
	...
	public void startSimulation() throws SimControlException {
		...
		boolean pb2=true;//--
		if (controlState == SimProcessStatus.IDLE) {	
			while (currentSimTime <= endSimTime && controlState != SimProcessStatus.STOPPED) {
				switch (controlState) {
				case IDLE:
				...
				case RUNNING:
						...
						if (simEventToTransform != null) {
							if (simMode == SimMode.ASTRONOMICAL) {							
								...
								//
								if (currentSimTime>=20 && pb1) {
									System.out.println("@SimModel 3- PAUZA- "+currentSimTime);
									pb1=false;
									pauseSimulation();//---
								}
								if (currentSimTime>=40 && pb2) {
									System.out.println("@SimModel 3- PAUZA- "+currentSimTime);
									pb2=false;
									pauseSimulation();//---
								}
				...
				case PAUSED:
					...
					int i=0;//--
					while (controlState == SimProcessStatus.PAUSED) {
						try {
							Thread.sleep(SimParameters.defaultAstronomicalTimeStep);
							i++;//--
							if (i==5) resumeSimulation();//--
						} catch (InterruptedException e) {
	...
	void setCurrentSimTime(double currentSimTime) {
